...
2. **Colors** - each colour's role and where it is allowed, referencing token values:
   ...
   action and critical highlights." Scarcity rules belong here.
3. **Typography** - the type strategy: which faces, chosen why, and how the hierarchy
   works. Note any face used for a specific job (data, labels, quotes).
4. **Layout** - the grid and spacing model (fixed max-width, fluid, asymmetric editorial),
   the spacing rhythm, container widths, and how density changes between sections.
5. **Elevation & Depth** - how hierarchy is conveyed: real shadow values, tonal layering,
   or explicitly flat with borders and contrast doing the work.
6. **Shapes** - the corner-radius language and its rationale, plus any other shape
   signature (cuts, seams, framing).
7. **Images** *(extension)* - the imagery type chosen for this direction and why it fits
   ...
   screen demo, diagram, or a deliberately image-light treatment), the treatment applied
   (crop, grade, framing), where images appear, and what each one depicts. See
   `images-and-icons.md`.
...
9. **Components** - per-component guidance: buttons and their variants, cards, badges and
   ...
   Cover states, not just resting appearance.
